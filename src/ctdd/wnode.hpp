...
namespace wnode{
	...
	public:
		T_WEIGHT weight;
	private:
		node::Node<T_WEIGHT>* p_node;

	public:

		inline node::Node<T_WEIGHT>* get_node() const noexcept {
			return p_node;
		}

		inline void set_node(node::Node<T_WEIGHT>* p_node) noexcept {
			p_node = p_node;
		}

		WNode(const WNode<T_WEIGHT>& other) noexcept {
			weight = other.weight;
			p_node = other.p_node;
		}

		WNode(WNode<T_WEIGHT>&& other) noexcept {
			node = nullptr;
			*this = std::move(other);
		}

		WNode(T_WEIGHT&& _weight, node::Node<T_WEIGHT>* _p_node) noexcept {
			weight = std::move(_weight);
			p_node = _p_node;
		}

		WNode() noexcept : p_node(nullptr) {}
		...
		static WNode<T_WEIGHT> get_wnode(T_WEIGHT&& wei, int order, succ_ls<T_WEIGHT>&& successors) {
			auto&& key = cache::unique_table_key<T_WEIGHT>(order, successors);

			auto&& p_find_res = node::Node<T_WEIGHT>::m_unique_table.find(key);

			if (p_find_res != node::Node<T_WEIGHT>::m_unique_table.end()) {
				// and another reference
				node::Node<T_WEIGHT>::ref_inc(p_find_res->second);
				...
				res.p_node = p_find_res->second;
				return res;
			}

			// a node of reference 1 is created.
			node::Node<T_WEIGHT>* p_node = new node::Node<T_WEIGHT>(order, std::move(successors));

			node::Node<T_WEIGHT>::m_unique_table[key] = p_node;
		...
		// 	return Node<T_WEIGHT>::is_multi_ref(node);
		// }

		// ~WNode() noexcept {
		// 	Node<T_WEIGHT>::ref_dec(node);
		// }
	};

	// template <class T_WEIGHT>
	// T_WEIGHT get_normalizer(const succ_ls<T_WEIGHT>& successors) {
	// 	if constexpr (std::is_same_v<T_WEIGHT, std::complex<double>>) {
	// 		int i_max = 0;
	...
	// 	}
	// 	else if constexpr (std::is_same_v<T_WEIGHT, CUDAcpl::Tensor>) {
	...
	template <class T_WEIGHT>
	inline bool is_equal(const WNode<T_WEIGHT>& a, const WNode<T_WEIGHT>& b) noexcept {
		return (a.get_node() == b.get_node() && weight::is_close(a.weight, b.weight));
	}
	...
	template <class T_WEIGHT>
	WNode<T_WEIGHT> normalize(const T_WEIGHT& wei, int order, succ_ls<T_WEIGHT>&& successors) {
		...
		T_WEIGHT weig_max{ get_normalizer(successors) };
		if (weight::is_exact_zero(weig_max)) {
			// in this case, all nodes in succesors should be the terminal node
			return WNode<T_WEIGHT>{ std::move(weig_max), nullptr };
		}

		T_WEIGHT reciprocal{ weight::reciprocal_without_zero(weig_max) };
		for (auto&& succ : successors) {
			...
			if (weight::is_zero(succ.weight)) {
				succ.weight = weight::zeros(wei);
				succ.set_node(nullptr);
			}
		}

		return WNode<T_WEIGHT>::get_wnode(weight::mul(weig_max, wei), order, std::move(successors));
	}
	...
	/// <returns></returns>
	template <class T_WEIGHT>
	WNode<T_WEIGHT> as_tensor_iterate(const Tensor& t,
		...
		auto&& dim_data = data_shape.size();
		if (dim_data == 0) {
			weight::element_to_weight<T_WEIGHT, Tensor>(t);
		...
		int axe_pos = split_pos - 1;

		//note: torch::chunk does not work here

		auto new_successors = succ_ls<T_WEIGHT>(data_shape[split_pos]);
		for (int i = 0; i < data_shape[split_pos]; i++) {
			auto&& indices = xt::xstrided_slice_vector(xt::all(), dim_data);
			indices[split_pos] = i;
			new_successors[i] = as_tensor_iterate<T_WEIGHT>(
				// UNSQUEEZE NEEDED !
				xt::xstrided_view(t, indices),
				data_shape, storage_order, depth + 1);
		}
		// normalize this depth
		res = normalize<T_WEIGHT>(weight::ones<T_WEIGHT>(), depth, std::move(new_successors));
		return res;
	}
	...
	// template <class T_WEIGHT>
	// CUDAcpl::Tensor to_CUDAcpl_iterate(const node::weightednode<T_WEIGHT>& w_node,
	...
	// 		}
	// 		else {
	// 			cache::CUDAcpl_table_key<T_WEIGHT> key;
	// 			bool do_cache = i->is_multi_ref();
	// 			if (do_cache) {
	// 				// first look up in the dictionary
	// 				key = cache::CUDAcpl_table_key<T_WEIGHT>(i->get_node(), data_shape);
	// 				//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	// 				cache::Global_Cache<T_WEIGHT>::CUDAcpl_cache.first.lock_shared();
	// 				auto&& p_find_res = cache::Global_Cache<T_WEIGHT>::CUDAcpl_cache.second.find(key);
	// 				if (p_find_res != cache::Global_Cache<T_WEIGHT>::CUDAcpl_cache.second.end()) {
	// 					uniform_tensor = p_find_res->second;
	// 					cache::Global_Cache<T_WEIGHT>::CUDAcpl_cache.first.unlock_shared();
	// 					//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	// 				}
	// 				else {
	// 					cache::Global_Cache<T_WEIGHT>::CUDAcpl_cache.first.unlock_shared();
	...
	// 					auto&& next_wnode = node::weightednode<T_WEIGHT>(weight::ones<T_WEIGHT>(para_shape), i->get_node());
	// 					uniform_tensor = to_CUDAcpl_iterate(next_wnode, para_shape, data_shape);

	// 					// add into the dictionary
	// 					//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	// 					cache::Global_Cache<T_WEIGHT>::CUDAcpl_cache.first.lock();
	// 					cache::Global_Cache<T_WEIGHT>::CUDAcpl_cache.second[key] = uniform_tensor;
	// 					cache::Global_Cache<T_WEIGHT>::CUDAcpl_cache.first.unlock();
	// 					//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	// 				}
	// 			}
	// 			else {
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// CUDAcpl::Tensor to_CUDAcpl(const node::weightednode<T_WEIGHT>& w_node,
	// 	const std::vector<int64_t>& para_shape,
	...
	// template <typename T_WEIGHT>
	// node::weightednode<T_WEIGHT> conj(const node::weightednode<T_WEIGHT>& w_node) {
	// 	node::Node<T_WEIGHT>* new_node;

	// 	if (w_node.get_node() == nullptr) {

	// 		if constexpr (std::is_same_v<T_WEIGHT, wcomplex>) {
	// 			return node::weightednode<T_WEIGHT>(std::conj(w_node.weight), nullptr);
	// 		}
	// 		else if constexpr (std::is_same_v<T_WEIGHT, CUDAcpl::Tensor>) {
	// 			return node::weightednode<T_WEIGHT>(CUDAcpl::conj(w_node.weight), nullptr);
	// 		}
	// 	}
	// 	else {
	// 		auto&& successors = w_node.get_node()->get_successors();
	// 		std::vector<node::weightednode<T_WEIGHT>> new_successors(successors.size());
	// 		for (int i = 0; i < successors.size(); i++) {
	// 			new_successors[i] = wnode::conj(successors[i]);
	// 		}

	// 		if constexpr (std::is_same_v<T_WEIGHT, wcomplex>) {
	// 			return node::weightednode<T_WEIGHT>::get_wnode(std::conj(w_node.weight), 
	// 				w_node.get_node()->get_order(), std::move(new_successors));
	// 		}
	// 		else if constexpr (std::is_same_v<T_WEIGHT, CUDAcpl::Tensor>) {
	// 			return node::weightednode<T_WEIGHT>::get_wnode(CUDAcpl::conj(w_node.weight),
	// 				w_node.get_node()->get_order(), std::move(new_successors));
	// 		}
	// 	}
	...
	// template <typename T_WEIGHT>
	// node::weightednode<T_WEIGHT> norm(const node::weightednode<T_WEIGHT>& w_node) {
	// 	node::Node<T_WEIGHT>* new_node;

	// 	if (w_node.get_node() == nullptr) {

	// 		if constexpr (std::is_same_v<T_WEIGHT, wcomplex>) {
	// 			return node::weightednode<T_WEIGHT>(std::norm(w_node.weight), nullptr);
	// 		}
	// 		else if constexpr (std::is_same_v<T_WEIGHT, CUDAcpl::Tensor>) {
	// 			return node::weightednode<T_WEIGHT>(CUDAcpl::norm(w_node.weight), nullptr);
	// 		}
	// 	}
	// 	else {
	// 		auto&& successors = w_node.get_node()->get_successors();
	...
	// 		if constexpr (std::is_same_v<T_WEIGHT, wcomplex>) {
	// 			return node::weightednode<T_WEIGHT>::get_wnode(std::norm(w_node.weight),
	// 				w_node.get_node()->get_order(), std::move(new_successors));
	// 		}
	// 		else if constexpr (std::is_same_v<T_WEIGHT, CUDAcpl::Tensor>) {
	// 			return node::weightednode<T_WEIGHT>::get_wnode(CUDAcpl::norm(w_node.weight),
	// 				w_node.get_node()->get_order(), std::move(new_successors));
	// 		}
	// 	}
	// }
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// inline weight::sum_nweights<T_WEIGHT> weights_normalize(const T_WEIGHT& weight1, const T_WEIGHT& weight2) {

	// 	T_WEIGHT renorm_coef;
	...
	// 	return weight::sum_nweights<T_WEIGHT>(
	// 		std::move(nweight1),
	// 		std::move(nweight2),
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// node::weightednode<T_WEIGHT> sum_iterate(
	// 	const node::weightednode<T_WEIGHT>& w_node1,
	// 	const node::weightednode<T_WEIGHT>& w_node2, const T_WEIGHT& renorm_coef, const std::vector<int64_t>& para_shape) {

	// 	node::weightednode<T_WEIGHT> res;
	// 	if (w_node1.get_node() == nullptr && w_node2.get_node() == nullptr) {
	// 		return node::weightednode<T_WEIGHT>(weight::mul((w_node1.weight + w_node2.weight), renorm_coef), nullptr);
	// 	}

	// 	// produce the unique key and look up in the cache
	// 	auto&& key = cache::sum_key<T_WEIGHT>(
	// 		w_node1.get_node(), w_node1.weight, w_node2.get_node(), w_node2.weight);

	// 	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	// 	cache::Global_Cache<T_WEIGHT>::sum_cache.first.lock_shared();
	// 	auto&& p_find_res = cache::Global_Cache<T_WEIGHT>::sum_cache.second.find(key);
	// 	auto found_in_cache = (p_find_res != cache::Global_Cache<T_WEIGHT>::sum_cache.second.end());

	// 	if (found_in_cache) {
	// 		res = p_find_res->second.get_weightednode();
	// 		cache::Global_Cache<T_WEIGHT>::sum_cache.first.unlock_shared();
	// 		//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	// 		///////////////////////////////////////////////////////////////////////
	// 		const node::weightednode<T_WEIGHT>* p_wnode_1, * p_wnode_2;
	...
	// 		auto&& new_successors = std::vector<node::weightednode<T_WEIGHT>>(p_wnode_1->get_node()->get_range());
	...
	// 				res = node::weightednode<T_WEIGHT>(p_wnode_1->weight + p_wnode_2->weight, p_wnode_1->get_node());
	// 				not_operated = false;
	// 			}
	...
	// 					);
	// 					auto&& next_wnode1 = node::weightednode<T_WEIGHT>(std::move(renorm_res.nweight1), i_1->get_node());
	// 					auto&& next_wnode2 = node::weightednode<T_WEIGHT>(std::move(renorm_res.nweight2), i_2->get_node());
	// 					*i_new = sum_iterate<T_WEIGHT>(next_wnode1, next_wnode2, renorm_res.renorm_coef, para_shape);
	// 				}
	// 				res = normalize<T_WEIGHT>(weight::ones<T_WEIGHT>(para_shape), p_wnode_1->get_node()->get_order(), std::move(new_successors));
	// 				not_operated = false;
	// 			}
	// 		}
	// 		if (not_operated) {
	...
	// 				auto&& next_wnode1 = node::weightednode<T_WEIGHT>(std::move(renorm_res.nweight1), i_1->get_node());
	// 				auto&& next_wnode2 = node::weightednode<T_WEIGHT>(std::move(renorm_res.nweight2), p_wnode_2->get_node());
	// 				*i_new = sum_iterate<T_WEIGHT>(next_wnode1, next_wnode2, renorm_res.renorm_coef, para_shape);
	// 			}
	// 			res = normalize<T_WEIGHT>(weight::ones<T_WEIGHT>(para_shape), p_wnode_1->get_node()->get_order(), std::move(new_successors));
	// 		}
	...
	// 		// cache the result
	// 		//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	// 		cache::Global_Cache<T_WEIGHT>::sum_cache.first.lock();
	// 		cache::Global_Cache<T_WEIGHT>::sum_cache.second[key] = res;
	// 		cache::Global_Cache<T_WEIGHT>::sum_cache.first.unlock();
	// 		//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// node::weightednode<T_WEIGHT> sum(
	// 	const node::weightednode<T_WEIGHT>& w_node1,
	// 	const node::weightednode<T_WEIGHT>& w_node2, const std::vector<int64_t>& para_shape) {
	// 	// normalize as a whole
	// 	auto&& renorm_res = weights_normalize(w_node1.weight, w_node2.weight);
	// 	auto&& next_wnode1 = node::weightednode<T_WEIGHT>(std::move(renorm_res.nweight1), w_node1.get_node());
	// 	auto&& next_wnode2 = node::weightednode<T_WEIGHT>(std::move(renorm_res.nweight2), w_node2.get_node());
	// 	return sum_iterate<T_WEIGHT>(next_wnode1, next_wnode2, renorm_res.renorm_coef, para_shape);
	// }
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// node::weightednode<T_WEIGHT> trace_iterate(const node::weightednode<T_WEIGHT>& w_node,
	// 	const std::vector<int64_t>& para_shape,
	// 	const std::vector<int64_t>& data_shape,
	...
	// 		}
	// 		return node::weightednode<T_WEIGHT>(w_node.weight * scale, nullptr);
	// 	}

	// 	node::weightednode<T_WEIGHT> res;

	// 	// first look up in the dictionary
	// 	auto&& key = cache::trace_key<T_WEIGHT>(w_node.get_node(), remained_ls, waiting_ls);

	// 	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	// 	cache::Global_Cache<T_WEIGHT>::trace_cache.first.lock_shared();
	// 	auto&& p_find_res = cache::Global_Cache<T_WEIGHT>::trace_cache.second.find(key);
	// 	if (p_find_res != cache::Global_Cache<T_WEIGHT>::trace_cache.second.end()) {
	// 		res = p_find_res->second.get_weightednode();
	// 		cache::Global_Cache<T_WEIGHT>::trace_cache.first.unlock_shared();
	// 		//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	// 				auto&& new_successors = std::vector<node::weightednode<T_WEIGHT>>(range);
	// 				if (order == remained_ls_pd[0].first) {
	// 					int index_val = 0;
	// 					auto&& i_new = new_successors.begin();
	...
	// 						next_waiting_ls[insert_pos].second = index_val;
	// 						succ_new = trace_iterate(node::weightednode<T_WEIGHT>(weight::ones<T_WEIGHT>(para_shape), w_node.get_node()),
	...
	// 					res = sum<T_WEIGHT>(res, *i, para_shape);
	// 				}
	// 				not_operated = false;
	// 			}
	// 		}
	...
	// 			auto&& new_successors = std::vector<node::weightednode<T_WEIGHT>>(w_node.get_node()->get_range());
	// 			auto&& i_new = new_successors.begin();
	...
	// 				}
	// 			}
	// 			res = normalize<T_WEIGHT>(weight::ones<T_WEIGHT>(para_shape), new_order[order], std::move(new_successors));
	// 		}
	...
	// 		// add to the cache
	// 		//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	// 		cache::Global_Cache<T_WEIGHT>::trace_cache.first.lock();
	// 		cache::Global_Cache<T_WEIGHT>::trace_cache.second[key] = res;
	// 		cache::Global_Cache<T_WEIGHT>::trace_cache.first.unlock();
	// 		//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// node::weightednode<T_WEIGHT> trace(const node::weightednode<T_WEIGHT>& w_node,
	// 	const std::vector<int64_t>& para_shape,
	// 	const std::vector<int64_t>& data_shape,
	...
	// /// </summary>
	// /// <typeparam name="T_WEIGHT"></typeparam>
	// /// <param name="w_node"></param>
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// node::weightednode<T_WEIGHT> slice_iterate(const node::weightednode<T_WEIGHT>& w_node,
	// 	const std::vector<int64_t>& para_shape,
	// 	const std::vector<int64_t>& data_shape,
	// 	const cache::pair_cmd& remained_ls, boost::unordered_map<node::Node<T_WEIGHT>*, node::weightednode<T_WEIGHT>>& slice_cache,
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// node::weightednode<T_WEIGHT> slice(const node::weightednode<T_WEIGHT>& w_node,
	// 	const std::vector<int64_t>& para_shape,
	// 	const std::vector<int64_t>& data_shape,
	// 	const cache::pair_cmd& remained_ls, const std::vector<int64_t> reduced_indices) {
	...
	// 	boost::unordered_map<node::Node<T_WEIGHT>*, node::weightednode<T_WEIGHT>> slice_cache{};
	...
	// /// <returns></returns>
	// template <class T_WEIGHT>
	// node::weightednode<T_WEIGHT> shift(const node::weightednode<T_WEIGHT>& w_node, int order_benchmark,
	// 	boost::unordered_map<node::Node<T_WEIGHT>*, node::Node<T_WEIGHT>*>& shift_cache) {
	...
	// template <class T_WEIGHT>
	// node::weightednode<T_WEIGHT> stack(const std::vector<node::weightednode<T_WEIGHT>>& nodes_ls, 
	// 	const std::vector<int64_t>& parallel_shape) {

	// 	std::vector<node::weightednode<T_WEIGHT>> new_successors{ nodes_ls };
	...
	// 	return normalize(weight::ones<T_WEIGHT>(parallel_shape), 0, std::move(new_successors));
	// }
	...
	// 	const node::Node<W1>* p_node_a, const std::vector<int64_t>& para_shape_a, 
	// 	const node::Node<W2>* p_node_b, const std::vector<int64_t>& para_shape_b,
	...
	// 	auto&& key = cache::cont_key<W1, W2>(p_node_a, para_shape_a, p_node_b, para_shape_b, remained_ls,
	...
	// 				res = contract_iterate<W1, W2>(succ.get_node(), para_shape_a, p_node_b, para_shape_b,
	...
	// 				res = contract_iterate<W1, W2>(p_node_a, para_shape_a, succ.get_node(), para_shape_b,
	...
	// 					[&](int i) {
	// 						return contract_iterate<W1, W2>(successors_a[i].get_node(), para_shape_a, p_node_b, para_shape_b,
	...
	// 					[&](int i) {
	// 						return contract_iterate<W1, W2>(p_node_a, para_shape_a, successors_b[i].get_node(), para_shape_b,
	...
	// 						[&](int i) {
	// 							next_b_waiting_ls[insert_pos].second = i;
	// 							return contract_iterate<W1, W2>(p_node_a, para_shape_a, p_node_b, para_shape_b,
	...
	// 								p_node_a, para_shape_a, p_node_b, para_shape_b, weight::ones_like(weight), para_shape_res,
	...
	// 	const node::weightednode<W1>& w_node_a, const std::vector<int64_t>& para_shape_a,
	// 	const node::weightednode<W2>& w_node_b, const std::vector<int64_t>& para_shape_b,
	// 	const std::vector<int64_t>& para_shape_res,
	// 	const std::vector<int64_t>& data_shape_a, const std::vector<int64_t>& data_shape_b,
